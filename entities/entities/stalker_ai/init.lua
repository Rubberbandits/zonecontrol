...
function ENT:STALKERNPC_EnemySpotted() end
...
function ENT:STALKERNPCInit(VEC,MTYPE,CAPS)
	...
	if(self.SNPCClass=="C_MONSTER_CONTROLLER") then
		function self:STALKERNPCIsFriendlyClass(NPC)
			if(NPC.SNPCClass=="C_MONSTER_CONTROLLER"||NPC.SNPCClass=="C_ZOMBIE"||
			NPC.VJ_NPC_Class=="CLASS_ZOMBIE"||NPC:Classify()==CLASS_ZOMBIE||NPC:Classify()==CLASS_HEADCRAB) then
				return true
			end
				
			return false
		end
	elseif(self.SNPCClass=="C_ZOMBIE") then
		function self:STALKERNPCIsFriendlyClass(NPC)
			if(NPC.SNPCClass=="C_MONSTER_CONTROLLER"||NPC.SNPCClass=="C_ZOMBIE"||
			NPC.VJ_NPC_Class=="CLASS_ZOMBIE"||NPC:Classify()==CLASS_ZOMBIE||NPC:Classify()==CLASS_HEADCRAB) then
				return true
	...
	elseif(self.SNPCClass=="C_MONSTER_BURER") then
		function self:STALKERNPCIsFriendlyClass(NPC)
			if(NPC.SNPCClass=="C_MONSTER_BURER") then
				return true
			end
...
function ENT:STALKERNPCIsEnemyPlayer(ENT2)
	...
	if( ENT2:IsPlayer() && ENT2:Alive() ) then
...
function ENT:STALKERNPCTryToFindEnemy(ignoreanim)
	...
	if (#TEMP_MyNearbyTargets>0) then 
		for T=1, #TEMP_MyNearbyTargets do
			...
			TEMP_NPC:GetClass()!="bullseye_strider_focus"&&TEMP_NPC:GetClass()!="scripted_target") then
				if((TEMP_NPC:IsNPC()||TEMP_NPC:IsPlayer())) then
					if(TEMP_NPC:IsNPC()||self:STALKERNPCIsEnemyPlayer(TEMP_NPC)) then
						...
						if(self:Visible(TEMP_NPC)) then	
							if(TEMP_NPC:GetClass()==self:GetClass()) then
								table.insert(TEMP_TeamMates,TEMP_NPC)
							end
	...
	if(IsValid(TEMP_NearestNpc)&&TEMP_NearestNpc!=nil&&TEMP_NearestNpc!=NULL&&TEMP_NearestNpc!=self) then
		if(self.PlayingAnimation==false||ignoreanim==true) then
			if(TEMP_NearestNpc!=self:GetEnemy()) then
				self:STALKERNPC_EnemySpotted(TEMP_NearestNpc,TEMP_TeamMates)
...
function ENT:STALKERNPCKill(dmginfo)
	...
	if(self.IsAlive==true) then
		...
		if(!(IsValid(TEMP_ATTACKER)&&TEMP_ATTACKER!=nil&&TEMP_ATTACKER!=NULL&&TEMP_ATTACKER:GetClass()=="npc_barnacle")) then
			if(GetConVar("ai_serverragdolls"):GetInt()==0&&self.ServersideRagdollOnly!=true) then
				...
				if(TEMP_Ragdoll:GetPhysicsObjectCount()>1) then
					if(self.RagdollModel==self:GetModel()||self.RagdollModel==nil) then
						...
						for P=0, TEMP_Ragdoll:GetPhysicsObjectCount()-1 do
							...
							if(IsValid(TEMP_Phys)) then
								local TEMP_Bone = TEMP_Ragdoll:TranslatePhysBoneToBone(P)
								local TEMP_BoneName = TEMP_Ragdoll:GetBoneName(TEMP_Bone)
								
								local TEMP_NewBone = self:LookupBone(TEMP_BoneName)
								
								if(TEMP_NewBone) then
									local TEMP_BoneMat = self:GetBoneMatrix(TEMP_NewBone)
									local TEMP_BonePos, TEMP_BoneAng = TEMP_BoneMat:GetTranslation(), TEMP_BoneMat:GetAngles()
					...
					local TEMP_DieTime = GetConVar("stalker_npc_corpse_dietime"):GetInt()
					
					if(TEMP_DieTime>0) then
						TEMP_Ragdoll:Fire("kill","",TEMP_DieTime)
					end
					
					TEMP_KillTime = 0.1
				else
					if(isfunction(self.NoRagdollCode)) then
						TEMP_KillTime = 0.01
						self:NoRagdollCode()
					end
				end
		...
		if(self.FakeDeathOnly==true) then
...
function ENT:OnTakeDamage(dmginfo)
	...
	dmginfo:GetDamageType()!=DMG_SONIC&&dmginfo:GetDamageType()!=DMG_POISON) then
		STALKERNPCBleed(self,dmginfo:GetDamage()/5,dmginfo:GetDamagePosition(),Angle(math.random(1,360),math.random(1,360),math.random(1,360)))
	end
	...
	if(self.PlayingAnimation==false) then
		...
		self:GetEnemy():GetPos():Distance(self:GetPos())>TEMP_RealAttacker:GetPos():Distance(self:GetPos()))) then
			if(TEMP_RealAttacker:IsPlayer()||(TEMP_RealAttacker:IsNPC())) then
				self:STALKERNPC_EnemySpotted(TEMP_RealAttacker,self)
