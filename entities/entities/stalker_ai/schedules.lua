...
function ENT:SelectSchedule( iNPCState )
	...
	if(!self.LastPosBehavior) then
		self.LastPosBehavior = SCHED_IDLE_WANDER
	end
	
	if(!self.LastPosFails) then
		self.LastPosFails = 0
	end
	...
	if(self.PlayingAnimation==false||self.PlayingGesture==true) then
		...
		(self:GetEnemy():IsNPC()||(self:GetEnemy():IsPlayer()&&self:GetEnemy():Alive()))) then
			...
			if(self.LastPosBehavior==SCHED_IDLE_WANDER) then
				if(self.LastPos:Distance(self:GetPos())<0.2) then
					self.LastPosFails = self.LastPosFails+1
				else
					self.LastPosFails = 0
				end
				
				if(self.LastPosFails>49) then
					self.LastPosBehavior = SCHED_RUN_RANDOM
				end
			elseif(self.LastPosBehavior==SCHED_RUN_RANDOM) then
				if(self.LastPos:Distance(self:GetPos())<0.2) then
					self.LastPosFails = self.LastPosFails+1
				else
					self.LastPosFails = self.LastPosFails-1
				...
				if(self.LastPosFails>99) then
					self.LastPosBehavior = SCHED_FORCED_GO
				elseif(self.LastPosFails<1) then
					self.LastPosBehavior = SCHED_IDLE_WANDER
				end
			elseif(self.LastPosBehavior==SCHED_FORCED_GO) then
				self.LastPosFails = self.LastPosFails-1
					
				if(self.LastPosFails<1) then
					self.LastPosBehavior = SCHED_IDLE_WANDER
				end
				
				if((self.LastPosFails/8)==math.floor(self.LastPosFails/8)) then
					self:SetLastPosition(self:GetPos()+(Vector(math.Rand(-100,100),math.Rand(-100,100),0):GetNormalized()*100))
				end
			end
			
			
			
			if(!self:IsCurrentSchedule(self.LastPosBehavior)) then
				self:SetSchedule(self.LastPosBehavior)
			end
		end
	end
