...
function SWEP:HandleRecoil()
	...
	if CLIENT then
		...
		ang.p = ang.p - self.Primary.RecoilAdd * 0.75 * Crouched * Moving * math.Remap(self.Owner:Aim(), 0, 100, 1, 0.5)
		ang.y = ang.y - self.Primary.RecoilAdd * math.Rand(-0.5 * Irons, 0.5 * Irons) * Crouched * Moving * math.Remap(self.Owner:Aim(), 0, 100, 1, 0.5)
		self.Owner:SetEyeAngles(ang)

	else

		self.Owner:ViewPunch(Angle(-self.Primary.RecoilAdd, self.Primary.RecoilAdd * math.Rand(-0.33 * Irons, 0.33 * Irons), 0) * 0.2 * Crouched * Moving * math.Remap(self.Owner:Aim(), 0, 100, 1, 0.5))
	...
	if self.Firearm then
		...
		if self:CanPrimaryAttack() then
			...
			if type(self.Primary.Sound) == "table" then
				self:PlaySound(self.Primary.Sound, 100, 100)
			else
				self:PlaySound(self.Primary.Sound, 100, 100)
			end
		...
		if tr.Hit then
			...
			if SERVER then
				...
				if tr.Entity:IsPlayer() then
					...
					netstream.Start(tr.Entity, "nFlashRed")
...
function SWEP:DrawHUD()
	if self:InScope() then
		...
		if false and render.GetDXLevel() >= 90 then
