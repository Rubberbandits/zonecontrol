...
if( CLIENT ) then
	...
	netstream.Hook( "nWakeUp", nWakeUp );
...
function meta:MakeRagdollClone()
	...
	rag:SetPos( self:GetPos() + Vector(0,0,2) );
...
function meta:WakeUp( spawn )
	...
	if( !spawn ) then
		...
		netstream.Start( nil, "nWakeUp", self );
...
function GM:ConsciousThink( ply )
	...
	if( CurTime() >= ply.ConsciousUpdate ) then
		...
		if kingston.blowout.get_var("Bool", "active_blowout") and ply:Consciousness() >= 90 then
			return
		end
