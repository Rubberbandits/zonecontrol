			...
			function CCP.RadioSelector.OK:DoClick()
				...
				if( val >= 0 ) then
					...
					if( val <= 999 ) then
						...
						GAMEMODE:AddChat( {[CB_ALL] = true, [CB_IC] = true}, "NewChatFont", Color( 200, 200, 200, 255 ), "Your radio channel is now " .. tostring( math.Round( val , 2 ) ) .. "." );
						
						netstream.Start( "nChangeRadio", val );
						
					else
						
						GAMEMODE:AddChat( {[CB_ALL] = true, [CB_IC] = true}, "NewChatFont", Color( 200, 0, 0, 255 ), "Highest channel is 999." );
						
					end
					
				else

					GAMEMODE:AddChat( {[CB_ALL] = true, [CB_IC] = true}, "NewChatFont", Color( 200, 0, 0, 255 ), "Lowest channel is 0." );
