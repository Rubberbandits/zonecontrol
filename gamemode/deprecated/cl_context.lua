...
function GM:GetCCOptions( ent, dist )
	...
	if( ent and ent:IsValid() ) then
		...
		elseif ent:GetClass() == "prop_physics" then	
			...
			if LocalPlayer():HasPermission("togglesaved") then
		...
		if charItems then
			for itemId,itemData in next, charItems do
				...
				if metaitem.AllowGender then
					if ent:Gender() == GENDER_FEMALE then
						mdl_str = string.StripExtension(mdl_str).."_f.mdl"
					end
				elseif metaitem.ScaleForGender and ent:Gender() == GENDER_FEMALE then
					scale = metaitem.ScaleForGender
				end
				
				local mdl = CCP.PlayerViewer.CharacterModel:InitializeModel(mdl_str, CCP.PlayerViewer.CharacterModel.Entity, scale)
