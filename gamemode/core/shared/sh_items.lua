...
hook.Add("CoreLoaded", "LoadItems", function()
	local path = string.format("%s/gamemode/items/base", GM.FolderName)
	...
	local path = string.format("%s/gamemode/items", GM.FolderName)
	local files = file.Find( path.."/*.lua", "LUA", "namedesc" );
	if( #files > 0 ) then
	...
	for name,data in next, GM.MetaBases do
		local path = string.format("%s/gamemode/items/%s", GM.FolderName, name)
		local files = file.Find( path.."/*.lua", "LUA", "namedesc" );
		for _,n in next, files do
			...
			ITEM.Base = ITEM.Base or name;
			
			if ITEM.Base == "weapon" then
				WEAPON_ALL[ITEM.Class] = true
			end
			
			s_BaseItem = table.Copy( GM.MetaBases[name] );
			table.Inherit( ITEM, s_BaseItem );
			ITEM.BaseClass = nil;
	...
	*/
	...
	local path = string.format("%s/gamemode/upgrades/base", GM.FolderName)
	local files = file.Find( path.."/*.lua", "LUA", "namedesc" );
	for _, v in next, files do
	...
	local path = string.format("%s/gamemode/upgrades", GM.FolderName)
	local files = file.Find( path.."/*.lua", "LUA", "namedesc" );
	for _, v in next, files do
	...
	local path = string.format("%s/gamemode/suit_variants", GM.FolderName)
	local files = file.Find( path.."/*.lua", "LUA", "namedesc" );
	for _, v in next, files do
