...
function GM:PMCreateInventory()
	...
	if CCP.PlayerMenu.InvButtonScroll and CCP.PlayerMenu.InvButtonScroll:IsValid() then
		CCP.PlayerMenu.InvButtonScroll:Clear()
	end
	
	CCP.PlayerMenu.InvButtons = { };
	CCP.PlayerMenu.SelectedItem = nil
	...
	CCP.PlayerMenu.InvDescContainer = vgui.Create( "DScrollPanel", CCP.PlayerMenu.ContentPane );
	CCP.PlayerMenu.InvDescContainer:SetPos( 420, 250 );
	CCP.PlayerMenu.InvDescContainer:SetSize( 220, 90 );
	function CCP.PlayerMenu.InvDescContainer:Paint( w, h )
	end
	
	CCP.PlayerMenu.InvDesc = vgui.Create( "DLabel", CCP.PlayerMenu.InvDescContainer );
	CCP.PlayerMenu.InvDesc:SetText( "No item selected." );
	CCP.PlayerMenu.InvDesc:SetPos( 0, 0 );
	...
	CCP.PlayerMenu.InvButtonScroll = vgui.Create("DScrollPanel", CCP.PlayerMenu.ContentPane)
	CCP.PlayerMenu.InvButtonScroll:SetPos( CCP.PlayerMenu.ContentPane:GetWide() - 110, 220 );
	CCP.PlayerMenu.InvButtonScroll:SetSize( 100, 150 );
	function CCP.PlayerMenu.InvButtonScroll:Paint( w, h )
	end
...
local function GenerateActions(i, item)
	local y = 0;

	for k,v in next, CCP.PlayerMenu.InvButtons do
		v:Remove()
		CCP.PlayerMenu.InvButtons[k] = nil
	end
	
	-- todo, implement dynamic item funcs in menu
	if i.functions then
		for m,n in next, i.functions do
			if( CCP.PlayerMenu["Button"..m] and CCP.PlayerMenu["Button"..m]:IsValid() ) then
				CCP.PlayerMenu["Button"..m]:Remove();
			end
			
			if !n.CanRun or !n.CanRun(item) then continue end
			...
			CCP.PlayerMenu["Button"..m].DoClick = function()
				netstream.Start( "ItemCallFunction", item:GetID(), m );
				item:CallFunction( m );
				GAMEMODE:PMUpdateInventory()
			end
			CCP.PlayerMenu["Button"..m]:PerformLayout();
			table.insert( CCP.PlayerMenu.InvButtons, CCP.PlayerMenu["Button"..m] );
			y = y + 25;
		end
	end
	
	if( CCP.PlayerMenu["ButtonDrop"] and CCP.PlayerMenu["ButtonDrop"]:IsValid() ) then
		CCP.PlayerMenu["ButtonDrop"]:Remove();
	end
	if( CCP.PlayerMenu["ButtonStockpile"] and CCP.PlayerMenu["ButtonStockpile"]:IsValid() ) then
		CCP.PlayerMenu["ButtonStockpile"]:Remove();
	end
	...
	if item:CanDrop() then
		if InStockpileRange(LocalPlayer()) then
			CCP.PlayerMenu["ButtonStockpile"] = vgui.Create( "DButton", CCP.PlayerMenu.InvButtonScroll );
			CCP.PlayerMenu["ButtonStockpile"]:SetFont( "CombineControl.LabelSmall" );
			CCP.PlayerMenu["ButtonStockpile"]:SetText( "Stockpile" );
			CCP.PlayerMenu["ButtonStockpile"]:SetPos( 0, y );
			CCP.PlayerMenu["ButtonStockpile"]:SetSize( 100, 20 );
			CCP.PlayerMenu["ButtonStockpile"].DoClick = function()
				if( item.CanDrop ) then
					if( item:CanDrop() ) then
			...
			CCP.PlayerMenu["ButtonStockpile"]:PerformLayout();
			table.insert( CCP.PlayerMenu.InvButtons, CCP.PlayerMenu["ButtonStockpile"] );
			y = y + 25;
		end
		
		if LocalPlayer():IsAdmin() then
			CCP.PlayerMenu["ButtonEdit"] = vgui.Create( "DButton", CCP.PlayerMenu.InvButtonScroll );
			CCP.PlayerMenu["ButtonEdit"]:SetFont( "CombineControl.LabelSmall" );
			CCP.PlayerMenu["ButtonEdit"]:SetText( "Edit" );
			CCP.PlayerMenu["ButtonEdit"]:SetPos( 0, y );
			CCP.PlayerMenu["ButtonEdit"]:SetSize( 100, 20 );
			CCP.PlayerMenu["ButtonEdit"].DoClick = function()
				if( item.CanDrop ) then
					if( item:CanDrop() ) then
			...
			CCP.PlayerMenu["ButtonEdit"]:PerformLayout();
			table.insert( CCP.PlayerMenu.InvButtons, CCP.PlayerMenu["ButtonEdit"] );
			y = y + 25;
		end
		
		CCP.PlayerMenu["ButtonDrop"] = vgui.Create( "DButton", CCP.PlayerMenu.InvButtonScroll );
		CCP.PlayerMenu["ButtonDrop"]:SetFont( "CombineControl.LabelSmall" );
		CCP.PlayerMenu["ButtonDrop"]:SetText( "Drop" );
		CCP.PlayerMenu["ButtonDrop"]:SetPos( 0, y );
		CCP.PlayerMenu["ButtonDrop"]:SetSize( 100, 20 );
		CCP.PlayerMenu["ButtonDrop"].DoClick = function()
			if( item.CanDrop ) then
				if( item:CanDrop() ) then
		...
		CCP.PlayerMenu["ButtonDrop"]:PerformLayout();
		table.insert( CCP.PlayerMenu.InvButtons, CCP.PlayerMenu["ButtonDrop"] );
		y = y + 25;
		...
		if LocalPlayer():HasCharFlag("X") and item.BulkPrice and InStockpileRange(LocalPlayer()) and GAMEMODE.SellToMenuEnabled and item:CanSell() then
			CCP.PlayerMenu["ButtonSell"] = vgui.Create( "DButton", CCP.PlayerMenu.InvButtonScroll );
			CCP.PlayerMenu["ButtonSell"]:SetFont( "CombineControl.LabelSmall" );
			CCP.PlayerMenu["ButtonSell"]:SetText( "Sell for "..GAMEMODE:CalculatePrice(item).." RU" );
			CCP.PlayerMenu["ButtonSell"]:SetPos( 0, y );
			CCP.PlayerMenu["ButtonSell"]:SetSize( 100, 20 );
			CCP.PlayerMenu["ButtonSell"].DoClick = function()
				if( item.CanDrop ) then
					if( item:CanDrop() ) then
			...
			CCP.PlayerMenu["ButtonSell"]:PerformLayout();
...
function GM:PMUpdateInventory()
	...
	for k, v in next, LocalPlayer().Inventory do
		...
		local i = GAMEMODE:GetItemByID( v:GetClass() );
		...
		if( i ) then
			...
			if v.GetBodygroupCategory then
				icon.Entity:SetBodygroup( v:GetBodygroupCategory(), v:GetBodygroup() );
			end
			if i.Base == "clothes" then
				local submats = v:GetItemSubmaterials()
				if submats then
					for m,n in next, submats do
						icon.Entity:SetSubMaterial( n[1], n[2] )
					end
				end
			else
				if i.ItemSubmaterials then
					for m,n in next, i.ItemSubmaterials do
						icon.Entity:SetSubMaterial( n[1], n[2] )
					end
				end
			end
			...
			function icon:DoClick()
				...
				CCP.PlayerMenu.InvModel:SetModel( v:GetModel() );
				if v.GetBodygroupCategory then
					CCP.PlayerMenu.InvModel.Entity:SetBodygroup( v:GetBodygroupCategory(), v:GetBodygroup() );
				end
				if i.Base == "clothes" then
					local submats = v:GetItemSubmaterials()
					if submats then
						for m,n in next, submats do
							CCP.PlayerMenu.InvModel.Entity:SetSubMaterial( n[1], n[2] )
						end
					end
				else
					if i.ItemSubmaterials then
				...
				CCP.PlayerMenu.InvTitle:SetText( v:GetName() );
				CCP.PlayerMenu.InvWeight:SetText( "Weight: " .. tostring(v:GetWeight()) );
				CCP.PlayerMenu.InvDesc:SetText( v:GetDesc() );
				...
				if( i.IconMaterial ) then CCP.PlayerMenu.InvModel.Entity:SetMaterial( i.IconMaterial ) end
				if( i.IconColor ) then CCP.PlayerMenu.InvModel.Entity:SetColor( i.IconColor ) end
	...
	if( CCP.PlayerMenu.SelectedItem ) then
		local item = LocalPlayer().Inventory[CCP.PlayerMenu.SelectedItem]
		if item then
			local metaitem = GAMEMODE:GetItemByID(item:GetClass())
			GenerateActions(metaitem, item)
		else
			self:PMResetText();
		end
	end
	
	CCP.PlayerMenu.InvWeightBar:SetProgress( LocalPlayer():GetWeight() / LocalPlayer():InventoryMaxWeight() );
	CCP.PlayerMenu.InvWeightBar:SetProgressText( "Weight: " .. LocalPlayer():GetWeight() .. "/" .. LocalPlayer():InventoryMaxWeight() );
...
function GM:PopulateMoveToStock( tbl )
	...
	for k,v in next, tbl do
		...
		CCP.StockpilesMenu.Stockpile[k].DoClick = function( self )
			...
			LocalPlayer():MoveToStockpile( CCP.PlayerMenu.SelectedItem, k );
